...
function defaultGenSid(ctx, length) {
    return uid_ts_1.uidSync(length);
}
/**
...
function sessionFactory(options = {}) {
    const { key = 'koa.sid', store = new stores_1.MemoryStore(key), reconnectTimeout = 10000, cookieOptions = { signed: true }, defer = false, rolling = false, allowEmpty = false, genSid = defaultGenSid, errorHandler = defaultErrorHanlder, valid = () => true, beforeSave = () => undefined, } = options;
